<?php
namespace IMPAKTT\ERP\Accounting;
...
class Ajax_Handler {
    ...
    function __construct() {
        ...
        $this->action( 'wp_ajax_erp-ac-quote-row-bulk-action', 'transaction_row_bulk_action_quote' );
    ...
    public function transaction_restore() {
        ...
        if ( $type == 'journal' ) {
            ...
            $url = erp_ac_quote_get_sales_url();
        }
     ...
     * @return void
     */
    public function transaction_row_bulk_action_quote() {
        ...
        switch ( $status ) {
            case 'delete':
                $update = erp_ac_quote_remove_transaction( $trns_id );
                break;

            case 'void':
                $update = erp_ac_quote_update_transaction( $trns_id, ['status' => $status] );
                break;

            case 'won':
                $update = erp_ac_quote_won_transaction( $trns_id, ['status' => $status]  );
                break;				
    ...
    public function transaction_form_submit() {
        ...
        if ( $postdata['type'] == 'sales' ) {
            $return_url = erp_ac_quote_get_sales_url( false );
