...
if ( $hook == 'accounting_page_erp-quote'  ) {
    $args = array(
        'type'       => ['quote'],
        'status'     => ['in' => ['draft','won', 'lost', 'awaiting_payment', 'awaiting_approval']],
        'output_by'  => 'array',
        'join'       => ['items'],
    ...
    $transactions = erp_ac_get_all_transaction_quote($args);
    ...
    foreach( $transactions as $key => $transaction ) {
		...
		if ($transaction['status'] == 'won') {
            ...
            $lost_amount = $lost_amount + $transaction['trans_total'];
            $lost     = $lost + 1;					
        }else if ( $transaction['status'] != 'won' &&   $transaction['status'] != 'lost') {
			$outstanding = $outstanding + $transaction['trans_total'];
        }
...
<div class="payment-stat-chart">
    <div class="payment-metrics">
        <span class="title"><?php _e( 'Amount', 'erp' ); ?></span>
    ...
    <div class="payment-metrics single-metric">
        <span class="title"><?php _e( 'Total Pending QUOTE', 'erp' ); ?></span>
